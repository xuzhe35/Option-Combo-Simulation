...
- **`processLegData(leg, simDate, ivOffset, ...)`** — Computes `T`, `calDTE`, `tradDTE`, `simIV`, `posMultiplier`, `costBasis` for a single leg.
- **`computeLegPrice(processedLeg, underlyingPrice, r)`** — BSM or intrinsic value (expired).
- **`computeSimulatedPrice(...)`** — Wraps `computeLegPrice` with Zero-Delta bypass: in Trial mode at T=0 with no IV offset, uses the live quote directly instead of BSM, eliminating fake P&L drift.

**Rule:** Never write `pos * 100 * price` anywhere outside `bsm.js`.

## 2. Time Convention: Calendar Days / 365
BSM `T = calDTE / 365.0` (matching TWS and industry standard). This correctly prices calendar spreads crossing weekends — trading-day-based `T = tradDTE / 252` underestimates weekend time value.
...
- Expected P&L normalized by total `nPaths` (1M), not clipped bin count — ensures full-spectrum tail risk is captured.
- MC worker is **explicitly triggered** via the `🔄 Recalculate` button, not on every slider drag.
...
- Rule-based engine that dynamically computes NYSE holidays for any year — no static data to maintain.
...
- Weekend observance: Saturday holidays → observed Friday; Sunday holidays → observed Monday.
...
- **Tech Debt Fix**: `ib_server.py` Deep OTM fallback chain improved: `(bid+ask)/2` → `modelGreeks.optPrice` → `marketPrice()`. Prevents stale last-trade quotes for illiquid options.
...
- **P0 Bug**: Fixed `importFromJSON` `calendarToTradingDays` misuse (single int → two date args).
- **SSOT**: Centralized Zero-Delta bypass into `bsm.js` → `computeSimulatedPrice()`.
...
### 2026-02-23
- Initial `bsm.js` SSOT refactoring (Sections 1–4 above).
