...
| 价外 10% 以上 Put 的实际抬升 vs ATM 抬升 | 基本相等（例：2025-04 +19.0 vs +17.2；2020-03 +34.8 vs +34.8） | 逐合约统一冲击点数是对的 |
| 各期限抬升相对约 30 天的比例 | 120 天 0.88，240 天 0.57，400 天 0.48（中位数） | √(30/DTE) 给 0.50 / 0.35 / 0.27，**明显低估远期**；(30/DTE)^0.25 给 0.71 / 0.59 / 0.52，更接近。默认衰减指数改为 0.25，可调，0.5 保留 |
| 前端 β（30 天 ATM 抬升 / 跌幅%） | 慢跌 −6%：0.7；三到四周跌 12–15%：1.3–1.7；四天跌 12.6%：2.3；2024-08 含 VIX 尖峰：2.9 | 默认 1.5 保留为中位起点；面板提示按速度选 β |

实现改动：`tenorDampingFactor(remaining, reference, exponent)`，新增「衰减指数」输入
（`stress-linked-iv-tenor-exponent`，默认 0.25，范围 0.05–1，按账本记忆），
状态行写 `按期限衰减 (30/剩余天)^0.25`。研究脚本随仓库保留，可对新的暴跌事件重跑。
...
| 假设 | 数据结论 | 落地 |
| --- | --- | --- |
| β 是常数 1.5 | 过原点回归：跌 2–5% 0.93，5–10% 1.05，>10% 1.4，多周暴跌 1.6；持有天数 1–40 天影响很小 | 新增「β 按跌幅自适应」（默认开）：按该点联动跌幅在表 [(≤5, 0.93), (7.5, 1.05), (12, 1.4), (≥20, 1.6)] 线性插值；手填 β 仍可用 |
| 各行权价加同样点数 | 价外 10–20% Put 的抬升只有 ATM 的 0.55 倍（中位，n=34），20%+ 也约 0.6 | 新增「价外折扣」（默认开）：距现货 ≤5% 取 1，≥10% 取 0.55，线性过渡；对两本账本同时生效 |
